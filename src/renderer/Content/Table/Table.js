  ...
  const getTickerInfo = () => {
    ...
    fetch(
      ...
      .then((result) => {
        // console.log(result["quoteResponse"]["result"][0])
        setYearIncome(
          result.quoteSummary.result[0].financialData.totalRevenue.raw
        );
      });

    fetch(
      'https://yfapi.net/v11/finance/quoteSummary/AAPL?lang=en&region=US&modules=summaryDetail',
      {
        headers: {
          accept: 'application/json',
          'X-API-KEY': 'l3qwKprfEf8kpp1kSGBDq9MFDBlcgY0WTWj20uGb',
      ...
      .then((result) => {
        // console.log(result["quoteResponse"]["result"][0])
        setDividendYield(
          result.quoteSummary.result[0].summaryDetail.dividendYield.raw
        );
      });
  };
